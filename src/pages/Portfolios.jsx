...
function PortfolioTable({ portfolios, eaPool, onSelect, selected }) {
  ...
  // ⚡ = i 10 portafogli della collezione che arrivano prima al funded nella
  // challenge di riferimento (backtest storico a partenza mobile, vedi
  // challenge_speed.py), tra quelli con P(passare) sopra la soglia minima.
  const fastSet = new Set(
    portfolios
      .filter(p => p.ch_meets_min_pass && p.ch_expected_days_to_funded != null)
      .sort((a, b) => a.ch_expected_days_to_funded - b.ch_expected_days_to_funded)
      .slice(0, 10)
      .map(p => p.rank)
  );
  ...
  const sorted = [...filtered].sort((a, b) => {
    ...
    // valori mancanti sempre in fondo, in entrambe le direzioni
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: "0.75rem", marginBottom: "1rem", flexWrap: "wrap" }}>
        ...
        {hasChallengeData && (
          <button
            onClick={() => { setSortKey("ch_expected_days_to_funded"); setSortDir("asc"); }}
            title="Ordina per tempo atteso al funded nella challenge di riferimento (ripartenze incluse)"
            style={{ padding: "0.25rem 0.6rem", fontSize: 12, cursor: "pointer",
                     background: sortKey === "ch_expected_days_to_funded" ? "var(--accent-dim)" : "var(--bg-elevated)",
                     border: "1px solid var(--accent)", borderRadius: "var(--radius-sm)",
                     color: "var(--accent)", fontWeight: 600 }}>
            ⚡ Più veloci in challenge
          </button>
        )}
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
          <thead>
            <tr style={{ borderBottom: "1px solid var(--border)" }}>
              ...
              {th("CH PASS%", "ch_pass_rate")}
              {th("CH GG MED", "ch_median_days")}
              {th("→FUNDED",  "ch_expected_days_to_funded")}
              {th("DR",       "_hrp_dr")}
              {th("ENB%",     "_hrp_enb")}
                ...
                >
                  <td style={{ padding: "0.5rem 0.75rem" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "0.4rem" }}>
                      ...
                      {fastSet.has(p.rank) && (
                        <span title={`Tra i 10 più veloci della collezione per arrivare al funded: ~${p.ch_expected_days_to_funded} giorni attesi (ripartenze incluse)`}
                              style={{ fontSize: 11, color: "var(--accent)", cursor: "help" }}>⚡</span>
                      )}
                      {hasWarning && (
                  ...
                  {/* Challenge di riferimento: backtest storico a partenza mobile */}
                  <td style={{ padding: "0.5rem 0.75rem", textAlign: "right", fontFamily: "var(--font-data)",
                               color: p.ch_pass_rate == null ? "var(--text-muted)"
                                    : p.ch_pass_rate >= 80 ? "var(--accent)"
                                    : p.ch_pass_rate >= 60 ? "var(--warning)" : "var(--danger)" }}>
                    {p.ch_pass_rate != null ? `${fmt(p.ch_pass_rate, 0)}%` : "—"}
                  </td>
                  <td style={{ padding: "0.5rem 0.75rem", textAlign: "right", fontFamily: "var(--font-data)",
                               color: "var(--text-secondary)" }}>
                    <span title={p.ch_median_days != null ? `Mediana ${p.ch_median_days}gg · P75 ${p.ch_p75_days}gg · P90 ${p.ch_p90_days}gg (giorni di calendario, solo tentativi riusciti)` : ""}
                          style={{ cursor: "help" }}>
                      {p.ch_median_days != null ? p.ch_median_days : "—"}
                    </span>
                  </td>
                  <td style={{ padding: "0.5rem 0.75rem", textAlign: "right", fontFamily: "var(--font-data)",
                               fontWeight: fastSet.has(p.rank) ? 700 : 400,
                               color: fastSet.has(p.rank) ? "var(--accent)" : "var(--text-primary)" }}>
                    <span title={p.ch_expected_days_to_funded != null
                                  ? `Giorni attesi fino al funded ripartendo dopo ogni bocciatura (≈${p.ch_expected_attempts} tentativi). Rischio: DD storico = ${Math.round((p.ch_opt_dd_frac || 0) * 100)}% del limite DD${p.ch_meets_min_pass ? "" : " — P(passare) sotto la soglia minima"}`
                                  : ""}
                          style={{ cursor: "help" }}>
                      {p.ch_expected_days_to_funded != null ? p.ch_expected_days_to_funded : "—"}
                      {p.ch_expected_days_to_funded != null && !p.ch_meets_min_pass && " ⚠"}
                    </span>
                  </td>
    ...
    <Card style={{ marginTop: "1.25rem" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start",
        ...
        <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
          ...
          {portfolio.ch_expected_days_to_funded != null && (
            <span title={`Challenge di riferimento simulata facendola partire in ${portfolio.ch_n_starts} giorni diversi del backtest, sulla sequenza reale dei giorni (periodi morti inclusi).\n` +
                         `P(passare) ${fmt(portfolio.ch_pass_rate, 0)}% · mediana ${portfolio.ch_median_days}gg · P75 ${portfolio.ch_p75_days}gg · P90 ${portfolio.ch_p90_days}gg\n` +
                         `Tempo atteso al funded (ripartenze incluse): ${portfolio.ch_expected_days_to_funded}gg\n` +
                         `Rischio ottimale: DD storico del portafoglio = ${Math.round((portfolio.ch_opt_dd_frac || 0) * 100)}% del limite su conto 100k.\nMoltiplicatore da applicare al parametro di rischio di OGNI EA rispetto al backtest (es. InpRiskMoney testato a 20k DD): ×${fmt(portfolio.ch_scale_vs_backtest, 4)}`}
                  style={{ cursor: "help" }}>
              <Badge value={`⚡ Challenge: ${fmt(portfolio.ch_pass_rate, 0)}% · ${portfolio.ch_median_days}gg med · ${portfolio.ch_expected_days_to_funded}gg al funded`}
                     type={portfolio.ch_meets_min_pass ? "positive" : "warning"} />
            </span>
          )}
